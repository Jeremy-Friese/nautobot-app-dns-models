...
- **Enabled** - Automatically increments the `soa_serial` field on a `DNSZone` by 1 whenever DNS-serving zone data changes (record create/update/delete, or modification of zone-level fields such as `name`, `ttl`, `filename`, or any SOA parameter). While enabled, manual changes to `soa_serial` through the API or GUI are rejected. See the [DNS Zone model documentation](../models/dnszone.md#soa-serial-auto-increment) for full details.

!!! warning "Incompatible with date-based serials"
    Do not enable this option if you manage `soa_serial` using a date-based scheme (e.g., `YYYYMMDDNN`). Auto-increment advances the counter by 1 on each change, which will quickly produce values that no longer reflect the date. This conflicts with any external tooling, scripts, or manual processes that set or expect date-formatted serials, and may cause unexpected ordering between serials generated by different methods.
